...
class UserFriendshipsControllerTest < ActionController::TestCase
	context "#index" do
		...
		context "when logged in"  do
			setup do
				@friendship1 = create(:pending_user_friendship, user: users(:rose), friend: create(:user, first_name: 'Pending', last_name: 'Friend'))
				@friendship2 = create(:accepted_user_friendship, user: users(:rose), friend: create(:user, first_name: 'Active', last_name: 'Friend'))
				@friendship3 = create(:requested_user_friendship, user: users(:rose), friend: create(:user, first_name: 'Requested', last_name: 'Friend'))
				@friendship4 = user_friendships(:blocked_by_rose)
			...
			should "display pending informtion on a pending friendship" do
				assert_select "#user_friendship_#{@friendship1.id}" do
					assert_select "em", "Friendship is pending."
				end
			end

			should "display date informtion on an accepted friendship" do
	...
	context "#accept" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:pending_user_friendship, user: users(:rose), friend: @friend)
				create(:pending_user_friendship, friend: users(:rose), user: @friend)
				sign_in users(:rose)
			end
	...
	context "#edit" do
		...
		context "when logged in" do
			setup do
				@user_friendship = create(:pending_user_friendship, user: users(:rose))
				sign_in users(:rose)
	...
	context "#destroy" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:rose))
				create(:accepted_user_friendship, friend: users(:rose), user: @friend)
